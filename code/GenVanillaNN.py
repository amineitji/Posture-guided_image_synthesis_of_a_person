...
class GenVanillaNN():
    ...
    def train(self, n_epochs=20):
        ...
        batch_size = min(32, max(8, len(train_dataset) // 20))
        print(f"[TRAIN] Train: {train_size} | Val: {val_size}")
        print(f"[TRAIN] Batch size adaptatif: {batch_size}")
        ...
        # Gestion compatibilité PyTorch (verbose existe seulement >= 1.13)
        try:
        ...
        for epoch in range(n_epochs):
            ...
            for i, (inputs, targets) in enumerate(train_loader):
                ...
                if (i + 1) % 10 == 0:
                    print(f"  [Epoch {epoch+1}/{n_epochs}] Batch {i+1}/{len(train_loader)} - Loss: {loss.item():.5f}")
            ...
            print(f"[EPOCH {epoch+1}/{n_epochs} TERMINÉ] Train Loss: {avg_train_loss:.5f} | Val Loss: {avg_val_loss:.5f}")
            print("-" * 60)
